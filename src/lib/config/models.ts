...
export type Model = {
	...
	apiModelId: string;
	/** Provider-imposed cap on images per single request. When undefined the
	 * app's default cap (DEFAULT_MAX_IMAGES_PER_MESSAGE in chat-view) applies.
	 * Populate this for vision models whose API rejects above a hard limit
	 * (Groq Llama 4 = 5, Mistral Pixtral / Large = 8) so the picker stops
	 * the user before they hit a cryptic provider 400. */
